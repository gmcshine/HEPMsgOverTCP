...
func main() {
/*
    fmt.Println(">>>> Usage: ./hep_send [msgNumSend] [restTimerMicroSec] [threadNum] [protocol] [dstIP] [dstPort]]<<<<"); 
    args := os.Args
    argNum := len(args)
...
*/
    ...
    fmt.Println(">>>> Usage: ./hep_send [-tls] [-dl] [-mn msgNumSend] [-rn restTimerMicroSec] [-tn threadNum] [-tu protocol] [-da dstIP] [-dp dstPort]]<<<<"); 
    createFlags()
    dumpConfig()
