...
#echo "4. 重命名服务证书"
cp ${CN}.key server.key
cp ${CN}.csr server.csr
cp ${CN}.crt server.crt

#echo "5. 重命名客户端证书"
mv ${CN}.key client.key
mv ${CN}.csr client.csr
mv ${CN}.crt client.crt
